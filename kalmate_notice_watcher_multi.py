...
- ✅ noticedetail 앵커 (<a class="noticedetail" name="SEQ">제목</a>) 우선 파싱
- NoticeView 링크 직접 탐지 + 실패 시 HTML에서 seq_no 후보 추출 → 상세페이지 접속해 제목/날짜 파싱
- 첫 실행(BASELINE) 알림 지원
...
def extract_seqs_from_html(html: str) -> List[str]:
    ...
    for pat in SEQ_FROM_ONCLICK_PATS:
        for m in pat.finditer(html):
            seqs.add(m.group(1))
    # 3) noticedetail에서 name="digits" 형태까지 커버(백업용 전체 검색)
...
def parse_notice_posts_from_html(session: requests.Session, html: str, current_list_url: Optional[str] = None) -> List[Post]:
    ...
    # 3) 실패 시: HTML에서 seq_no 후보 찾아 상세페이지 접속
